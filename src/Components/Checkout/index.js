...
export default function Checkout() {
    ...
    <>
      <Container>
        ...
        <AllProducts>
          <Title>
            <h1>
              {totalPrice === 0 && myproducts.length === 0
                ? Cleancart({ totalPrice, myproducts })
                : "Seus Produtos"}
            </h1>
          </Title>
          {myproducts.map((product, index) => {
            ...
            return (
              <ItemProducts
                today={product.today}
                key={index + Date.now() + index + Date.now()}
                id={product.id}
                img={product.image}
                text={product.title}
                price={product.price}
    ...
    return (
      <CssFooter>
        <Price>
          <span>
            <h1>
              Total:<s className="numberItems">(itens {numberItems})</s>{" "}
            </h1>{" "}
            <h1 className="blue">R$ {price}</h1>
          </span>
          <span></span>
        </Price>
        <button
          onClick={ async () => {
            try {
              await axios.post("https://projeto-np-commerce-back.herokuapp.com/senddone", myproducts, config)
      ...
      <Products>
        ...
        <div className="right">
          <div className="rightPrice">
            <span className="">
              <s className="green">%{11}</s>
              <s className="risco">R${price}</s>
            </span>
  ...
  async function deleteProduct({ today }) {
    if (window.confirm("deseja realmente tirar do seu carrinho?")) {
      try {
        await axios.delete(
          `https://projeto-np-commerce-back.herokuapp.com/deletemyproducts/${today}`,
          config
        );
        const promise = axios.get("https://projeto-np-commerce-back.herokuapp.com/myproducts", config);
        promise.then((res) => {
